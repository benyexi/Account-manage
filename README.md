...
- 👤 **游客入口 `/`**：开放注册（一律注册为游客）、登录，登录后进入个人中心——查看自己的注册信息、编辑昵称 / 邮箱、修改密码
- ⚙️ **管理员后台 `/admin`**：管理所有账号（新增、编辑、删除、启用 / 禁用、重置密码、设置角色），未登录或游客访问会被自动送回 `/`

其他能力：

- 🔐 scrypt 密码哈希 + Bearer Token 会话（12 小时有效期）
- 🔎 按用户名 / 昵称 / 邮箱搜索，按角色、状态筛选，分页展示
- 📊 统计面板：总账号数、管理员、游客、正常、已禁用
